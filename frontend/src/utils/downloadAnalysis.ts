...
// Extended AudioAnalysis type to handle both formats
interface ExtendedAudioAnalysis {
  // Legacy format from backend agent
  assessment?: string[];
  scores?: {
...
export function formatAnalysisAsText(analysisResults: AnalysisResponse): string {
  ...
  if (analysisResults.agentResults) {
    ...
    if (agentResults.audioAnalysis) {
      ...
      // Check if it's the new format from Python service
      const audioData = agentResults.audioAnalysis as ExtendedAudioAnalysis;
      if (audioData.results) {
        const results = audioData.results;
        sections.push("Audio Analysis Results:");

        if (results.Confidence) {
          sections.push(`  • Confidence: ${results.Confidence.Score}/10`);
          sections.push(`    ${results.Confidence.Feedback}`);
        }

        if (results.Emotion) {
          sections.push(`  • Emotion: ${results.Emotion.Score}/10`);
          sections.push(`    ${results.Emotion.Feedback}`);
        }

        if (results.Fluency) {
          sections.push(`  • Fluency: ${results.Fluency.Score}/10`);
          sections.push(`    ${results.Fluency.Feedback}`);
        }

        if (audioData.duration_seconds) {
          sections.push(`  • Duration: ${audioData.duration_seconds} seconds`);
        }
        sections.push("");
      }
      // Legacy format from backend agent
